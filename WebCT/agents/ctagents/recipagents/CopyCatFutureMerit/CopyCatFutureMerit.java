package ctagents.recipagents.CopyCatFutureMerit;
...
public class CopyCatFutureMerit extends RecipAgent{
    ScoringUtility SU;
...
//                FL.updateTypeProbs(constants.prospectiveMeritProbWeights, cgs, OppPerGameId, MyPerGameId, MyPerGameId, OppPerGameId, offer, merit, false);
...
//            RL.updateTypeProbs(constants.prospectiveMeritProbWeights, cgs, ProposerId, ResponderId, ProposerId, ResponderId, myOffer, merit, accepted);
            ...
            updateMerit(ResponderId, FL.getMerit(cgs, ProposerId, ResponderId, ResponderId, ProposerId, myOffer, accepted, SU) );
            updateMerit(ProposerId, FL.getMerit(cgs, ProposerId, ResponderId, ProposerId, ResponderId, myOffer, accepted, SU) );
        }
    }
    
    public Object strategy(Object o) {
        ArrayList<ChipSet> mostBeneficialExchange = strategy(cgs, MyPerGameId, OppPerGameId, constants, FL, merit, logr, SU);
    ...
    public static ArrayList<ChipSet> strategy(GameStatus gs, int ProposerId, int ResponderId, RecipConstants constants,
            FMBRLogic FL, double[] merit, Logger logr, ScoringUtility SU) {
...
//                logr.info("New set of weights");
                ...
                aScore += weightProb.get(0) * FL.socialPref(gs, ProposerId, ResponderId, ProposerId, ResponderId, 
                        Offer, true, merit, weights, constants.prospectiveMeritCoeffsProposer.get(0), 1, SU);
            }
    ...
    public Boolean RespondStrategy(GameStatus gs, ChipSet proposal, int ProposerId, int ResponderId) {
        ...
        updateMerit(ProposerId, FL.getMerit(gs, ProposerId, ResponderId, ProposerId, ResponderId, proposal, null, SU) );
        ...
        for(ArrayList<Double> weightProb : constants.prospectiveMeritProbWeights ){
            ...
            aScore += weightProb.get(0) * FMBRLogic.socialPref(cgs, ProposerId, ResponderId, MyPerGameId, OppPerGameId, 
                    proposal, true, merit, weights, constants.prospectiveMeritCoeffsResponder.get(0), 1, SU);
            rScore += weightProb.get(0) * FMBRLogic.socialPref(cgs, ProposerId, ResponderId, MyPerGameId, OppPerGameId, 
                    proposal, false, merit, weights, constants.prospectiveMeritCoeffsResponder.get(0), 1, SU);
        }
        ...
        updateMerit(ResponderId, FL.getMerit(gs, ProposerId, ResponderId, ResponderId, ProposerId, proposal, accept, SU) );
        return(accept);
    }
