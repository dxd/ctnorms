 ...
 * @author ilke
 */
public class FutureMeritBestResponseAgent implements RecipAgentAdaptor, Observer {
    ...
    FMBRLogic FMBRLogic = new FMBRLogic();
    ...
    public FutureMeritBestResponseAgent() {
...
//                FMBRLogic.updateTypeProbs(constants.retroProbWeights, cgs, OppPerGameId, MyPerGameId, MyPerGameId, OppPerGameId, offer, merit, false);
                for(ArrayList<Double> pw : constants.prospectiveMeritProbWeights)
                    System.out.println("prob prop is: " + pw.get(0)); 
                
                boolean offerResponse = RespondStrategy(cgs, offer, OppPerGameId, MyPerGameId, round);
...
//            FMBRLogic.updateTypeProbs(constants.retroProbWeights, cgs, ProposerId, ResponderId, ProposerId, ResponderId, myOffer, merit, accepted);
            for(int i = 0; i < constants.prospectiveMeritProbWeights.size(); i++ )
                System.out.println("prob: " + constants.prospectiveMeritProbWeights.get(i).get(0) );
            
            updateMerit(ResponderId, FMBRLogic.getMerit(cgs, ProposerId, ResponderId, ResponderId, ProposerId, myOffer, accepted, SU) );
            updateMerit(ProposerId, FMBRLogic.getMerit(cgs, ProposerId, ResponderId, ProposerId, ResponderId, myOffer, accepted, SU) );
        }
    }
    ...
    public void phaseAdvanced(Phases ph) {
        ...
        if(phaseName.equals("Offer Phase")) {
            ...
            {   
                ...
                ArrayList<ChipSet> exchange = strategy( cgs, MyPerGameId, OppPerGameId, constants, FMBRLogic, merit, SU, round );
                ChipSet senderChips;
    ...
    public static ArrayList<ChipSet> strategy(GameStatus gs, int ProposerId, int ResponderId, RecipConstants constants,
            FMBRLogic RL, double[] merit, ScoringUtility SU, int round) {
        ...
        ArrayList<ArrayList<Double>> probCoeffs = constants.prospectiveMeritCoeffsProposer.get(0);

        int horizon = probCoeffs.size()   - round ;
        if (horizon<0)
            horizon = 0;
        ...
        for(ArrayList<ChipSet> exchange:allExchanges) {
            ...
            for(ArrayList<Double> weightProb : constants.prospectiveMeritProbWeights ){
            
                tweights = weightProb.subList(1, weightProb.size());
                
//                aScore += weightProb.get(0) * SU.getOfferScore(Offer, ProposerId);
               
                aScore += weightProb.get(0) * RL.socialPref(gs, ProposerId, ResponderId, ProposerId, ResponderId,
                        Offer, true, merit, tweights, constants.prospectiveMeritCoeffsProposer.get(0), horizon, SU);
                rScore += weightProb.get(0) * RL.socialPref(gs, ProposerId, ResponderId, ProposerId, ResponderId, 
                        Offer, false, merit, tweights, constants.prospectiveMeritCoeffsProposer.get(0), horizon, SU);

                pAccept += weightProb.get(0) * RL.probOfferAccepted(gs, ProposerId, ResponderId, ProposerId, ResponderId, 
                        Offer, merit, tweights, constants.prospectiveMeritCoeffsProposer.get(0), horizon, SU);
    ...
    public Boolean RespondStrategy(GameStatus gs, ChipSet proposal, int ProposerId, int ResponderId, int round) {
        ...
        updateMerit(ProposerId, FMBRLogic.getMerit(gs, ProposerId, ResponderId, ProposerId, ResponderId, proposal, null, SU) );
        ...
        int horizon = constants.prospectiveMeritCoeffsProposer.size() - round ;
        ...
        for(ArrayList<Double> weightProb : constants.prospectiveMeritProbWeights ){
            ...
            aScore += weightProb.get(0) * FMBRLogic.socialPref(cgs, ProposerId, ResponderId, MyPerGameId, OppPerGameId, 
                    proposal, true, merit, weights, constants.prospectiveMeritCoeffsResponder.get(0), horizon, SU);
            rScore += weightProb.get(0) * FMBRLogic.socialPref(cgs, ProposerId, ResponderId, MyPerGameId, OppPerGameId, 
                    proposal, false, merit, weights, constants.prospectiveMeritCoeffsResponder.get(0), horizon, SU);
        ...
        updateMerit(ResponderId, FMBRLogic.getMerit(gs, ProposerId, ResponderId, ResponderId, ProposerId, proposal, accept, SU) );
        return(accept);
    }
