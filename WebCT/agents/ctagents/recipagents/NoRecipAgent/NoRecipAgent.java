...
package ctagents.recipagents.NoRecipAgent;
...
public class NoRecipAgent implements RecipAgentAdaptor{
    ...
    public void onReceipt(DiscourseMessage dm) {
        ...
        if(cgs.getMyPlayer().getRole().equals("Responder")){
            if(dm instanceof BasicProposalDiscourseMessage) {
                ...
                for(ArrayList<Double> pw : constants.NoRecipProbweights)
                ...
                try {
                    ...
                    Logger.getLogger(NoRecipAgent.class.getName()).log(Level.SEVERE, null, ex);
                }
            ...
            updateTypeProbs(constants.NoRecipProbweights, cgs, ProposerId, ResponderId, MyPerGameId, OppPerGameId, myOffer, accepted);

            for(int i = 0; i < constants.NoRecipProbweights.size(); i++ )
                System.out.println("prob: " + constants.NoRecipProbweights.get(i).get(0) );
        }
    }
...
//                System.out.println("-------------------------------------------------");
               ...
               // double acceptscore = SU.getOfferScore(offer, ProposerId);
                ...
                for(ArrayList<Double> weightProb : constants.NoRecipProbweights ){
                ...
                aScore += weightProb.get(0) * socialPref(gs, ProposerId, ResponderId, ProposerId, ResponderId, offer,
                                           PropDefaultScore, OppDefaultScore,   tweights.get(0), tweights.get(1)   );
...
//
    ...
    for(ArrayList<Double> weightProb : constants.retroProbWeights ){
        tweights = weightProb.subList(1, weightProb.size());
            ...
            rScore += weightProb.get(0) * socialPref(cgs, MyPerGameId, OppPerGameId, MyPerGameId, OppPerGameId,
                    proposal, PropDefaultScore, OppDefaultScore,   tweights.get(0), tweights.get(1)    );
    ...
    private static double socialPref(GameStatus gs, int ProposerId, int ResponderId,
                ...
                double nnP, double nnR, double wIB, double wAU){
        ...
        if(perGameId == ProposerId){
            ...
            for( ArrayList<Double> l : probWeights ){
                ...
                responderPayoff = socialPref(gs, ProposerId, ResponderId, perGameId, opponentId, offer,
                            MyDefaultScore, OppDefaultScore, tweights.get(0), tweights.get(1));
